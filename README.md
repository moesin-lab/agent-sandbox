...
| `./runtime/home` | `/home/node` | CLI 登录态、shell 配置、用户级工具 |
| `./runtime/logs` | `/runtime/logs` | 日志 |
| `./runtime/state` | `/runtime/state` | compose 模式等状态 |
...
`sandbox` 默认以 `node` 用户运行，关闭 passwordless sudo，并在 compose 层启用 `no-new-privileges`、`cap_drop: ALL`、PID/CPU/内存上限。默认资源上限可以通过 `.env` 调整：
...
`ANTHROPIC_API_KEY` / `OPENAI_API_KEY` 没设也行，CLI 会回退到 oauth 流程；oauth 凭据落在持久化的 `runtime/home` 卷里，下次启动复用。`claude` 二进制本身也会在首次启动时下载到 `runtime/home/.local/bin`，之后不再跟镜像重建绑定；要 pin 版本可以在启动前设置 `CLAUDE_RELEASE_TAG`。
